...
def display(request):
    ...
    if movies_list:
        return render(request, 'ex05/display_moviesOSM.html', locals())
    else:
        return HttpResponse("No data available")
...
def remove(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and request.POST['select'][0]:
            Movies.objects.filter(episode_nb=request.POST['select'][0]).delete()

    movies_list = Movies.objects.all().order_by('episode_nb')
    if movies_list:
        return render(request, 'ex05/remove_movieOSM.html', locals())
    else:
        return HttpResponse("No data available")
