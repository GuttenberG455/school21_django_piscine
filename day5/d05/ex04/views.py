...
def display(request):
    ...
    try:
        ...
        if conn and not conn.closed:
            conn.close()
    if movies_list:
        return render(request, 'ex04/display_moviesSQL.html', locals())
    else:
        return HttpResponse("No data available")
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid() and request.POST['select']:
                sql_string = ''
                curr = conn.cursor()
                curr.execute(f"DELETE FROM ex04_movies WHERE episode_nb={request.POST['select']};")
                conn.commit()
        cursor = conn.cursor()
        form = Form()
        try:
        ...
        if conn and not conn.closed:
            conn.close()
    if movies_list:
        return render(request, 'ex04/remove_movieSQL.html', locals())
    else:
        return HttpResponse("No data available")
